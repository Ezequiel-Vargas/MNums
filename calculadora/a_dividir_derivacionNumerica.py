...
def derivacion(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Validar que la función sea evaluable en los puntos necesarios
            puntos_a_validar = [x0]
            if metodo == 'hacia_adelante':
                puntos_a_validar.append(x0 + h)
            elif metodo == 'hacia_atras':
                puntos_a_validar.append(x0 - h)
            elif metodo == 'central':
                puntos_a_validar.extend([x0 + h, x0 - h])

            # Validar todos los puntos
            for punto in puntos_a_validar:
                evaluarFuncion(f, x, punto)

            # Calcular derivada analítica para comparación
            try:
                derivada_simbolica = sp.diff(f, x)
                derivadaAnalitica = evaluarFuncion(derivada_simbolica,x, x0)
            except Exception as e:
                raise ValueError(f"Error al calcular la derivada analítica: {str(e)}")

            # Métodos de derivación numérica con evaluación mejorada
            if metodo == 'hacia_adelante':
                # Método de diferencia hacia adelante
                f_x0 = evaluarFuncion(f, x, x0)
                f_x0_h = evaluarFuncion(f, x, x0 + h)
                derivadaNumerica = (f_x0_h - f_x0) / h

                procedimiento = [
                    f"Función: f(x) = {funcion}",
                    f"Función procesada: f(x) = {f}",
                    f"Punto x0: {x0}",
                    f"Tamaño de paso h: {h}",
                    f"f(x0) = f({x0}) = {f_x0}",
                    f"f(x0 + h) = f({x0 + h}) = {f_x0_h}",
                    f"Derivada numérica (hacia adelante): f'({x0}) = (f({x0 + h}) - f({x0})) / {h} = {float(derivadaNumerica)}",
                    f"Derivada analítica: f'({x0}) = {derivadaAnalitica}"
                ]

            elif metodo == 'hacia_atras':
                # Método de diferencia hacia atrás
                f_x0 = evaluarFuncion(f, x, x0)
                f_x0_menos_h = evaluarFuncion(f, x, x0 - h)
                derivadaNumerica = (f_x0 - f_x0_menos_h) / h
                ...
                procedimiento = [
                    ...
                    f"Tamaño de paso h: {h}",
                    f"f(x0) = f({x0}) = {f_x0}",
                    f"f(x0 - h) = f({x0 - h}) = {f_x0_menos_h}",
                    f"Derivada numérica (hacia atrás): f'({x0}) = (f({x0}) - f({x0 - h})) / {h} = {float(derivadaNumerica)}",
                    f"Derivada analítica: f'({x0}) = {derivadaAnalitica}"
                ]

            elif metodo == 'central':
                # Método de diferencia central
                f_x0_mas_h = evaluarFuncion(f, x, x0 + h)
                f_x0_menos_h = evaluarFuncion(f, x, x0 - h)
                derivadaNumerica = (f_x0_mas_h - f_x0_menos_h) / (2 * h)
                ...
                procedimiento = [
                    ...
                    f"Derivada numérica (central): f'({x0}) = (f({x0 + h}) - f({x0 - h})) / (2*{h}) = {float(derivadaNumerica)}",
                    f"Derivada analítica: f'({x0}) = {derivadaAnalitica}"
                ]

            # Calcular error relativo
            if abs(derivadaAnalitica) > 1e-10:  # Evitar división por cero
                error = abs((derivadaNumerica - derivadaAnalitica) / derivadaAnalitica) * 100
            else:
                error = abs(derivadaNumerica - derivadaAnalitica) * 100  # Error absoluto si derivada es muy pequeña

            procedimiento.append(f"Error porcentual: {error:.6f}%")

            # Resultado final
            resultado = float(derivadaNumerica)
